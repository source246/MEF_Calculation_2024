...
FOSSIL_TECH_TO_FUEL = {
    ...
    "Nuclear": ("nuclear", "Kernenergie"),   # NEU
}
...
def exportable_blocks_for_zone(
    ...
    if "Nuclear" in gen_z_row.index:
        mw_raw = float(gen_z_row.get("Nuclear", 0.0))
        if np.isfinite(mw_raw) and mw_raw > 0:
            # SRMC korrekt berechnen:
            fuel_th = fuel_prices_row.get("nuclear_eur_mwh_th", 5.5)
            co2     = fuel_prices_row.get("co2_eur_t", 0.0)
            eta_eff = 0.33  # Standard η = 33 %
            varom   = 9.0   # €/MWh_e
            srmc = (fuel_th + co2 * EF_LOOKUP_T_PER_MWH_TH["Kernenergie"]) / eta_eff + varom
            candidates.append(("Kernenergie", srmc, mw_raw, eta_eff, zone))
    ...
    for tech, (pk, ef_name) in FOSSIL_TECH_TO_FUEL.items():
        ...
        d   = (nei_dists.get(zone, {}).get(ef_name) or nei_dists.get(ef_name) or DEFAULT_NEI_DISTS[ef_name])
