...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--imp_tol_mw", type=float, default=5.0)
    p.add_argument("--anchor_margin", type=float, default=0.0)
    # Kopplung / Preisanker
    ...
    return p
 # =========================
 # Patch 1: Anchor-Filter & Utilities (nur für Seitenwahl)
 # =========================
FOSSIL_FUELS = {
     "Hard coal","Lignite","Gas CCGT","Gas OCGT","Gas conventional",
     "Light oil","Heavy oil","Oil","Coal","Gas"
 }
NON_ANCHOR_FUELS = {
     "EE","Wind","Solar","Nuclear","Waste",
     "Hydro Water Reservoir","Hydro Pumped Storage",
     "Run-of-River","RoR","Reservoir","PSP"
 }
 
def _is_mustrun_block(b):
     """Heuristik: Flag oder sehr niedriges Gebot ⇒ Must-run."""
     return getattr(b, "is_mustrun", False) or (getattr(b, "bid", 9e9) <= 1.5)
 
def _fuel_of(b):
     return getattr(b, "fuel", getattr(b, "fuel_group", ""))
 
def make_anchor_stack(blocks):
     """
     Für die *Seitenwahl* nur potenziell preissetzende, flexible fossile Blöcke
     ohne Must-run zulassen. EE/MU/Reservoir/PSP bleiben außen vor.
     """
     out = []
     for b in (blocks or []):
         f = _fuel_of(b)
         if f in NON_ANCHOR_FUELS:
             continue
         if f not in FOSSIL_FUELS:
             continue
...
def marginal_srmc_for_anchor(blocks, required_mw):
     """SRMC der marginalen Einheit im Anchor-Stack (vereinfachte Aggregation)."""
     import math
     if not blocks or required_mw <= 1e-9:
         return math.nan
     bs = sorted(blocks, key=lambda b: float(getattr(b, "srmc", 9e9)))
     rem = float(required_mw)
     marg = bs[-1]
     for b in bs:
...
def _fee_from_gen_row(gen_row: pd.Series) -> float:
    cols = ["Solar","Wind Onshore","Wind Offshore","Hydro Run-of-river and poundage"]
    return float(pd.to_numeric(gen_row.reindex(cols).fillna(0.0), errors="coerce").sum())
...
def main(args):
    ...
    # gleich nach dem Laden von de_gen:
    de_gen_raw = de_gen.copy()
    ...
    for t in idx:
        ...
        # --- Preise & Fuels für Zeitpunkt t (p_de MUSS vor Kopplungscheck gesetzt sein) ---
        p_de = float(nei_prices.loc[t, "price_DE_LU"]) if (t in nei_prices.index and "price_DE_LU" in nei_prices.columns) else np.nan
        fp_row = fuel_prices.loc[t] if t in fuel_prices.index else None
        # Gekoppelte Zonen für Zeitpunkt t aus deinem Cluster-Mapping holen
        coupled_neighbors = clusters.get(t, [])  # z.B. ["NL","AT","FR"] oder []

        # --- Flüsse (nur gekoppelte Zonen berücksichtigen) ---
        ...
        # ================= Patch 1: Import-Gate + Anchor-Stacks (nur für Seitenwahl) =================
        tol_mw = float(getattr(args, "imp_tol_mw", 5.0))         # Nettoimport-Toleranz
        epsilon_price = float(getattr(args, "epsilon", 0.50))    # Preisgleichheitsfenster (für Kopplung)
        anchor_margin = float(getattr(args, "anchor_margin", 0)) # optionaler Puffer

        # gekoppelte Zonen für t
        coupled_neighbors = clusters.get(t, [])  # z.B. ["NL","AT","FR"] oder []

        # (1) Preiskopplung mit mind. einer gekoppelten Zone?
        coupled_equal = False
        if "nei_prices" in locals() and np.isfinite(p_de) and coupled_neighbors:
        ...
        # (2) Anchor-Stacks für Seitenwahl bauen (Import vs. Inland)
        # HINWEIS: falls 'domestic_flex_blocks' anders heißt, hier anpassen!
        anchor_dom = make_anchor_stack(domestic_flex_blocks)
        imp_anchor_blocks = []
        if coupled_equal and (net_imp > tol_mw) and coupled_neighbors:
            for z in coupled_neighbors:
                z_blocks = exportable_blocks_for_zone(z)
                imp_anchor_blocks.extend(make_anchor_stack(z_blocks))

        # (3) Anchor-SRMCs vergleichen (nur wenn Import-Kandidaten vorhanden)
        if coupled_equal and (net_imp > tol_mw) and imp_anchor_blocks and anchor_dom:
            RL_anchor = max(0.0, RL_after_psp)  # nimm deine präferierte RL-Stufe
            srmc_dom_anchor = marginal_srmc_for_anchor(anchor_dom, RL_anchor)
            srmc_imp_anchor = marginal_srmc_for_anchor(imp_anchor_blocks, RL_anchor)
            import_relevant = (srmc_imp_anchor + 1e-9 < srmc_dom_anchor - anchor_margin)
        else:
        ...
        # Variante A (robust mit reindex): erst Zeile -> Serie, dann numerisch + Summe
        row_de = de_gen_raw.reindex([t]).iloc[0]
        fee_de = float(pd.to_numeric(row_de.reindex(fee_cols), errors="coerce").sum())

        price_ok = (np.isfinite(p_de) and (p_de <= float(getattr(args, "ee_price_threshold", 5.0))))
        # EE-Surplus weich (dein bisheriges Flag)
        ee_surplus = (price_ok and (RL5 <= 1e-6))

        # --- EE-Überschuss-Definition (streng: Preis < 1 UND keine Residuallast nach FEE) ---
        FEE_raw = float(de_gen_raw[fee_cols].reindex([t]).sum(axis=1).iloc[0]) if fee_cols else 0.0
        RL1_raw = max(L - FEE_raw, 0.0)  # Residuallast ohne Skalierung prüfen
        ee_surplus_strict = (np.isfinite(p_de) and (p_de < 1.0) and (RL1_raw <= 1e-6))

        if ee_surplus_strict:
            # -> sofort als EE-Überschuss markieren und restliche Logik überspringen
            marginal_side  = "DE"
            marginal_label = "EE_surplus_price_lt1"
            marginal_fuel  = "EE"
            marginal_eta   = np.nan
            marginal_srmc  = 0.0
            mef_gpkwh      = 0.0
            ...
            results.append({
                ...
                "residual_domestic_fossil_MW": 0.0,
                "flag_psp_price_setting": False,
                "FEE_MW": FEE,
                "ND_EXTRA_MW": 0.0,
                "RL_after_FEE_MW": 0.0,
                "RL_after_MU_WASTE_MW": 0.0,
                "RL_after_MU_NUC_MW": 0.0,
                "RL_after_MU_BIO_MW": 0.0,
                "RL_after_FOSSIL_MU_MW": 0.0,
                "RL_after_PSP_MW": 0.0,
                "RL_after_RES_MW": 0.0,
                "BIO_MW": 0.0,
                "WASTE_MW": 0.0,
            ...
            debug_rows.append({
                ...
                "ND_MW": 0.0,
                "Load_MW": L,
                "LIGNITE_MUSTRUN_ENFORCED_MW": 0.0,
                "DEBUG_rule_psp_price_setting": False,
            })
            continue  # <- jetzt korrekt: innerhalb der for t in idx-Schleife

        # keep scaled version for arithmetic
        ...
        # --------- Kopplung: gepoolter Export-Stack aller gekoppelten Zonen ---------
        ...
        if (import_marg_srmc is not None) and coupling_active:
            ...
            # --- EE-Überschuss (gemeinsam gekoppelte Zonen) NACH Seitenwahl ---
            # Bedingung: Preis < Schwelle und gemeinsame Restlast ≤ 0
            ee_thr = float(getattr(args, "ee_price_threshold", 0.0))
            if np.isfinite(p_de) and (p_de < ee_thr):
                fee_techs = ["Solar","Wind Onshore","Wind Offshore","Hydro Run-of-river and poundage"]
            
                # DE/LU: FEE (RAW!) und Last
                fee_de_joint = 0.0
                if any(c in de_gen_raw.columns for c in fee_techs):
                    fee_de_joint = float(
                        pd.to_numeric(
                            de_gen_raw.reindex([t])[ [c for c in fee_techs if c in de_gen_raw.columns] ],
                            errors="coerce"
                        ).sum(axis=1).fillna(0.0).iloc[0]
                    )
                load_de_joint = float(load_by_zone.get("DE_LU", pd.Series(dtype="float64")).get(t, np.nan))
            
                joint_fee  = fee_de_joint
                joint_load = load_de_joint if np.isfinite(load_de_joint) else 0.0
            
                # Gekoppelte Zonen aufsummieren
                for z in coupled_neighbors:
                    # FEE in Zone z
                    if (z in gen_by_zone) and (t in gen_by_zone[z].index):
                        cols = [c for c in fee_techs if c in gen_by_zone[z].columns]
                        if cols:
                            joint_fee += float(
                                pd.to_numeric(gen_by_zone[z].reindex([t])[cols], errors="coerce")
                                .sum(axis=1).fillna(0.0).iloc[0]
                            )
                    # Last in Zone z
                    if z in load_by_zone:
                        lv = float(load_by_zone[z].get(t, np.nan))
                        if np.isfinite(lv):
                            joint_load += lv
            
                joint_residual = max(joint_load - joint_fee, 0.0)
            
                if joint_residual <= 1e-6:
                    # EE setzt (gemeinschaftlich) den Preis → auf EE überschreiben
                    marginal_side  = "DE"                # oder "COUPLED" wenn du ein neues Label willst
                    marginal_label = "EE_surplus_joint"  # klares Debug-/Analyse-Label
                    marginal_fuel  = "EE"
                    marginal_eta   = np.nan
                    marginal_srmc  = 0.0
                    mef_gpkwh      = 0.0
                    DEBUG_rule_ee_joint_surplus = True
            # --- Ende EE-Überschuss (joint) ---


        # --------- Peaker-Override (wie gehabt, optional) ----------
        if getattr(args, "peak_switch", False):
            thr1, thr2 = _parse_two_floats(getattr(args, "peak_price_thresholds", "180,260"))
            if np.isfinite(p_de) and (RL7 > 0.0) and (p_de >= thr1):
