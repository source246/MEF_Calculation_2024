...
def main(args):
    ...
    for t in idx:
        ...
        # Stufe 1: RL nach FEE
        RL_fee = max(L - FEE, 0.0)
        
        # Stufe 2: RL nach FEE minus (Biomasse + Waste + Nuclear)
        ND_ext_local = bio_flex_de + waste_t + ND_EXTRA  # ND_EXTRA ist hier nur "Nuclear"
        RL_ndext = max(RL_fee - ND_ext_local, 0.0)

        fee_covers     = (L <= FEE + 1e-6)
        nondisp_covers = (L <= (FEE + ND_EXTRA) + 1e-6)
        DEBUG_rule_psp_price = False   # init, wird ggf. unten gesetzt
        ...
        # Biomasse-MU gehört ebenfalls in die Mustrun-Stufe (leistungsbasiert hier als Gen-Näherung)
        mustrun_de_total += float(bio_mu_de)
        residual_domestic_fossil = max(RL_ndext - mustrun_de_total, 0.0)
        ...
        psp_avail = float(
            de_gen.get("Hydro Pumped Storage", pd.Series(0.0, index=de_gen.index))
                  .reindex([t]).fillna(0.0).iloc[0]
        ...
        de_reservoir_avail = float(
            de_gen.get("Hydro Water Reservoir", pd.Series(0.0, index=de_gen.index))
                  .reindex([t]).fillna(0.0).iloc[0]
        ) if "Hydro Water Reservoir" in de_gen.columns else 0.0
        if (residual > 0.0) and (de_reservoir_avail > 0.0) and (residual <= de_reservoir_avail + 1e-6) and np.isfinite(p_de):
            marginal_side  = "DE"
        ...
        results.append({
            ...
            "RL_after_FEE_MW": RL_fee,
            "RL_after_NDext_MW": RL_ndext,
            "BIO_MW": bio_raw,
            "WASTE_MW": waste_t,
