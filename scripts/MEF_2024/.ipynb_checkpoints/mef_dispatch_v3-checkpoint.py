...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--neighbor_fleet", default=None,
                   help="CSV mit Nachbar-Fleet (zone,fuel,eta/heat_rate,capacity_mw) → zonale η-Dists + Kapazitätsmaske")
    p.add_argument("--fossil_mustrun_mode",
               choices=["off","min_all","min_peak","min_peak_monthly"],
               default="min_peak",
               help="Mindesterzeugung (echtes Minimum) für Gas/Steinkohle/Öl: "
                    "min_all=globales Jahresminimum; "
                    "min_peak=separat für Peak/Offpeak; "
                    "min_peak_monthly=monatlich getrennt + Peak/Offpeak. "
                    "off=keine fossile Mustrun-Minderung")
    # optional: nur bestimmte Fuels berücksichtigen
    p.add_argument("--fossil_mustrun_fuels",
                   default="Erdgas,Steinkohle,Heizöl schwer",
                   help="Kommagetrennte Brennstoffe, für die Mindesterzeugung angesetzt wird "
                        "(Mapping wie EF_LOOKUP: Erdgas, Steinkohle, Heizöl schwer, Braunkohle?). "
                        "Braunkohle ist i.d.R. separat geregelt – hier typischerweise NICHT reinschreiben.")
    ...
    p.add_argument("--price_tol", type=float, default=30.0,
                   help="Toleranz SRMC vs. Day-Ahead-Preis (€/MWh) bei --price_anchor threshold")
    # Nachbarn: Effizienz-Modelle
    p.add_argument("--nei_eta_mode", choices=["mean","bounds","mc"], default="mean",
                   help="mean=Flottenmittel; bounds=min/max; mc=Normalverteilung mit Clipping")
    p.add_argument("--nei_eta_json", default=None,
                   help="JSON mit η-Parametern je Fuel (global oder je Zone)")
        # Fossile Mustrun-Anteile (optional; 0..1). Werden von der Residuallast abgezogen.
    ...
    p.add_argument("--nei_mc_draws", type=int, default=50, help="Ziehungen bei --nei_eta_mode mc")
    p.add_argument("--neighbor_capacity", default=None,
                   help="CSV: zone,fuel,capacity_mw → maskiert Fuels ohne installierte Leistung")
    # Dispatch-Details
    ...
    p.add_argument("--mustrun_lignite_q", type=float, default=0.20, help="Nur für capacity: Anteil Lignite-Kapazität")
    p.add_argument("--mustrun_quantile",  type=float, default=0.20, help="Nur für gen_quantile: Quantil reale Lignite-Gen")
...
def exportable_blocks_for_zone(
    ...
    # Roh-Residual vor Kappung: <0 ⇒ EE-Überschuss in der Zone
    ...
    # Reservoir-Hydro als regelbar (SRMC≈0)
...
def compute_fossil_min_profiles(
    ...
    peak_hours: str,
    mode: str,            # "off"|"min_all"|"min_peak"|"min_peak_monthly"
) -> Tuple[pd.Series, Dict[str, pd.Series]]:
    """
    Liefert:
      total_min: Series (h-stündlich) mit Summe der fossilen Mindesterzeugung
      min_by_fuel: dict[fuel]->Series mit h-stündlicher Mindesterzeugung je Fuel
    Wir lesen echte Minima aus der realen Erzeugung 2024, je nach Aggregation.
    """
    ...
    # Mapping Tech -> Fuelname wie in EF_LOOKUP
    TECH2FUEL = {
        "Fossil Gas": "Erdgas",
        "Fossil Hard coal": "Steinkohle",
        "Fossil Oil": "Heizöl schwer",
        # "Fossil Brown coal/Lignite": "Braunkohle",   # i.d.R. separat behandelt
    }
    # Welche Tech-Spalten existieren?
    ...
    if mode == "min_all":
        # Ein globales Minimum je Fuel über das ganze Jahr
        for tech in tech_cols:
            ...
            min_by_fuel[f] = pd.Series(m, index=idx, dtype="float64")

    elif mode == "min_peak":
        # Zwei Werte je Fuel: Peak-Minimum und Offpeak-Minimum
        pk_mask = is_peak(idx)
        op_mask = ~pk_mask
        for tech in tech_cols:
            f = TECH2FUEL[tech]
            ...
            m_pk = float(s[pk_mask].min()) if pk_mask.any() else 0.0
            m_op = float(s[op_mask].min()) if op_mask.any() else 0.0
        ...
        # Monatlich × (Peak/Offpeak) wechselseitig eigene Minima
        months = sorted(idx.unique().month)
        pk_mask_all = is_peak(idx)
        out_by_fuel = {f: pd.Series(0.0, index=idx, dtype="float64") for f in fuels_select}
        for tech in tech_cols:
            f = TECH2FUEL[tech]
            if f not in fuels_select: continue
            s = pd.to_numeric(gen_df[tech], errors="coerce").fillna(0.0)
            ...
            for m in months:
                mask_m = (idx.month == m)
                pk_m = mask_m & pk_mask_all
                op_m = mask_m & (~pk_mask_all)
                m_pk = float(s[pk_m].min()) if pk_m.any() else 0.0
                m_op = float(s[op_m].min()) if op_m.any() else 0.0
                out[ pk_m] = m_pk
                out[ op_m] = m_op
            out_by_fuel[f] = out
        min_by_fuel = out_by_fuel

    # Negatives/NaN vermeiden
    for f in min_by_fuel:
        v = pd.to_numeric(min_by_fuel[f], errors="coerce").fillna(0.0)
        v = v.clip(lower=0.0)
        min_by_fuel[f] = v
...
def main(args):
    ...
    for t in idx:
        ...
        # EE-Überschuss (ohne Handel) hat Priorität
        ee_surplus = (residual_domestic_fossil <= 1e-6)

        # --- DE: fossile Mustrun von der Residuallast abziehen (optional) ---
        de_mustrun_shares = fossil_mustrun_shares_for_DE(args)
        # Wir nutzen Kapazitäten aus der DE-Fleet (cap_base) pro EF-Key:
        # Summe(Kapazität je EF) * Share * therm_avail -> MW Mustrun
        ...
        # Domestic marginal (falls Residual>0)
        ...
        # Cluster & gekoppelter Preis
        cluster = clusters.get(t, ["DE_LU"])
        p_de = float(nei_prices.loc[t, "price_DE_LU"]) if t in nei_prices.index else np.nan

        # Import-Stack nur, wenn gekoppelt & Nettoimport >0
        marginal_import_label = None
        import_marg_srmc = None
        import_marg_mef  = None
        if (not ee_surplus) and (net_imp > 0.0) and (len(cluster) > 1):
            I = net_imp  # Importbedarf DE
            stack_all = []
            for c in imp_cols:
                mw_imp_from_c = float(flows.loc[t, c]) if t in flows.index else 0.0
                if mw_imp_from_c <= 1e-6:
                    continue
                z = imp_to_zone[c]
                if z not in cluster or z not in gen_by_zone or t not in gen_by_zone[z].index:
                    continue
            ...
            for (fuel, srmc, mw, eta, z) in stack_all:
                ...
                I_remaining -= take
                marg_block = (fuel, srmc, eta, z)  # letzter genutzter Block (marginal)
            ...
            if marg_block is not None:
                ...
                if fuel_m in ("Reservoir Hydro", "EE"):
                    import_marg_mef  = 0.0
                    lbl = "Reservoir Hydro" if fuel_m == "Reservoir Hydro" else "EE_surplus"
        ...
        # Seiten-Entscheid (Preis-Anchoring / SRMC-Vergleich)
        marginal_side = "DE"
        ...
        if choose_side == "IMPORT":
            ...
            # EE-Überschuss DE (ohne Handel): immer vorrangig → MEF≈0, Handel egal
            if ee_surplus:
                marginal_side  = "DE"
                ...
                marginal_srmc  = 0.0
                mef_gpkwh      = 0.0
                choose_side    = "EE"   # Marker: verhindert späteres Überschreiben
        ...
        results.append({
            ...
            "residual_after_trade_MW": residual,  # (optional, aber hilfreich fürs Debuggen)
        })
        debug_rows.append({
            "timestamp": t,
