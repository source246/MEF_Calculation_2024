...
def exportable_blocks_for_zone(
    ...
    if residual_raw <= -1e-6:
        ...
        # ... nachdem nuclear_surplus berechnet wurde:
    ...
    # Flex: Pumped Storage im Nachbarland (preisfolgend, arbitragegetrieben)
    if "Hydro Pumped Storage" in gen_z_row.index:
        mw_psp = float(gen_z_row.get("Hydro Pumped Storage", 0.0))
        if np.isfinite(mw_psp) and mw_psp > 0:
            # PSP ist preissetzend → SRMC ~ 0..Preis-Median; hier 0 ansetzen, MEF=0
            candidates.append(("Hydro Pumped Storage", 0.0, mw_psp, 1.0, zone))
