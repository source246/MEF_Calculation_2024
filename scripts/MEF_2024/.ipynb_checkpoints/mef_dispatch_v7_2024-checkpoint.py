...
def compute_net_load_from_df(df: pd.DataFrame) -> pd.Series:
    ...
    # Stündlich ist bereits durch read_csv_auto_time erledigt → nur aufräumen
...
def load_neighbor_load(path_dir: str, zone: str) -> pd.Series:
    # Dateien heißen bei dir: load_{ZONE}_2024.csv im Jahresordner (z. B. out_load/2024)
    ...
    # Deine Dateien haben z. B. 'timestamp_utc'/'timestamp_cec'/'timestamp_brussels'
    time_cols = ["timestamp_utc", "timestamp_cec", "timestamp_brussels"]
    df = read_csv_auto_time(str(candidates[0]), time_cols=time_cols)
    net = compute_net_load_from_df(df)  # nimmt 'ActualTotalLoad_MW'
    return net
...
EF_LOOKUP_T_PER_MWH_TH = {
    "Erdgas": 0.201, "Steinkohle": 0.335, "Braunkohle": 0.383,
    "Heizöl schwer": 0.288, "Heizöl leicht / Diesel": 0.266,
    "Kernenergie": 0.0
}
...
FOSSIL_TECH_TO_FUEL = {
    ...
    "Nuclear": ("nuclear", "Kernenergie"),   # NEU: Nuklear als regelbar im Export-Stack
}

PRICE_COLS = ["gas_eur_mwh_th","coal_eur_mwh_th","lignite_eur_mwh_th","oil_eur_mwh_th",
              "nuclear_eur_mwh_th",  # NEU
              "co2_eur_t"]
...
def map_fuel_to_price_and_ef(raw: str):
    t = _norm(raw)
    if "nuclear" in t or "kern" in t:                       # NEU
...
def load_fleet(path: str, eta_col: Optional[str]) -> pd.DataFrame:
    ...
    namecol = next((c for c in df.columns if "Anzeige-Name" in c oder "Name der Einheit" in c or "Name" in c), idcol)
...
def compute_unit_srmc_series(fleet, fuel_prices, varom_map):
    srmc_by_unit = {}
    ...
    for _, r in fleet.iterrows():
        ...
        if r["ef_key"] == "Kernenergie" and varom == 0.0:
            varom = 9.0  # €/MWh_e – Default, per JSON überschreibbar
...
DEFAULT_NEI_DISTS = {
    ...
    "Kernenergie":{"mean":0.33,"std":0.02,"min":0.30,"max":0.35},  # NEU (Range)
}
...
def exportable_blocks_for_zone(
    ...
    Regelbar: Reservoir Hydro (SRMC≈0) + fossile Fuels inkl. Nuklear.
    ...
    # 2) Sammle regelbare Kandidaten (Reservoir + regelbare Fuels) mit MW
    candidates = []  # (fuel, srmc, mw, eta, zone)
    
    # EE-Überschuss dieser Zone als exportierbarer Block (SRMC≈0, MEF=0)
    ...
    # Regelbare Thermaltechnologien (inkl. Nuclear)
    for tech, (pk, ef_name) in FOSSIL_TECH_TO_FUEL.items():
        ...
        d   = (nei_dists.get(zone, {}).get(ef_name) or nei_dists.get(ef_name) or DEFAULT_NEI_DISTS.get(ef_name))
        if d is None:
            continue
        m,s,lo,hi = d["mean"], d["std"], d["min"], d["max"]
        eta_eff = m if mode != "mc" else float(np.mean(truncated_normal(m, s, lo, hi, size=draws)))
    
        # vO&M: für Nuklear 9 €/MWh_e (Default), sonst 0
        varom_nei = 9.0 if ef_name == "Kernenergie" else 0.0
    
        srmc = (fuel_th + co2 * EF_LOOKUP_T_PER_MWH_TH.get(ef_name, 0.30)) / max(eta_eff, 1e-6) + varom_nei
...
def main(args):
    ...
    for t in idx:
        ...
        # --- NEU: Early-Exit NUR bei echtem EE-Überschuss ohne Import ---
        ee_surplus = (
        ...
        if (residual <= 1e-6) and not (net_imp > 0.0 and coupling_active):
            results.append({
                ...
                "mef_g_per_kwh": 0.0,
                "marginal_side": "DE",
                "marginal_label": "EE_surplus" if ee_surplus else "no_domestic_need_after_trade",
            ...
            debug_rows.append({
                ...
                "DE_eta": np.nan,
                "DE_srmc": np.nan,
                "IMPORT_stack_srmc_marg": np.nan,   # hier noch nichts berechnet
