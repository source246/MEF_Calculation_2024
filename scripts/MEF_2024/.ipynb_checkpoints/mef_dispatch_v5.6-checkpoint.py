...
def main(args):
    ...
    for t in idx:
        ...
        # --- EE-Überschuss-Definition (Preis < 1 UND keine Residuallast nach FEE) ---
        ...
        if ee_surplus_strict:
            ...
            continue  # <-- ganz wichtig: Rest der Logik für diese Stunde überspringen
...
DEBUG_rule_ee_joint_surplus = False

if (import_marg_srmc is not None) and coupling_active:
    # DE-Kandidat aus Einheitensuche, falls noch nicht gesetzt
    ...
    if args.price_anchor in ("closest","threshold") and np.isfinite(p_de):
        ...
        if np.isfinite(marginal_srmc):
            cand.append(("DE", abs(marginal_srmc - p_de)))
    ...
    # ---------- EE-Überschuss (gemeinsame gekoppelte Zonen) NACH der Seitenwahl ----------
    # Bedingung: Preis < Schwelle UND gemeinsame Restlast (Load - FEE_raw) ≤ 0
    ee_thr = float(getattr(args, "ee_price_threshold", 5.0))
    if np.isfinite(p_de) and (p_de < ee_thr):
        fee_techs = ["Solar", "Wind Onshore", "Wind Offshore", "Hydro Run-of-river and poundage"]

        # Hilfsfunktion: FEE_raw aus einer Zonen-Gen-Zeile holen (als Series!)
        def _fee_raw_from_row(row_ser: pd.Series) -> float:
            cols = [c for c in fee_techs if c in row_ser.index]
            if not cols:
                return 0.0
            return float(pd.to_numeric(row_ser.reindex(cols), errors="coerce").sum())

        # DE/LU
        fee_de_joint = 0.0
        if t in de_gen_raw.index:
            fee_de_joint = _fee_raw_from_row(de_gen_raw.reindex([t]).iloc[0])
        ...
        # gekoppelte Nachbarn addieren
        for z in coupled_neighbors:
            # FEE_raw
            if (z in gen_by_zone) and (t in gen_by_zone[z].index):
                joint_fee += _fee_raw_from_row(gen_by_zone[z].reindex([t]).iloc[0])
            # Load
        ...
        if joint_residual <= 1e-6:
            # gemeinsamer EE-Überschuss → EE setzt Preis
            marginal_side  = "DE"                 # oder eigenes Label wie "COUPLED"
            marginal_label = "EE_surplus_joint"
            marginal_fuel  = "EE"
            marginal_eta   = np.nan
            marginal_srmc  = 0.0
            mef_gpkwh      = 0.0
            DEBUG_rule_ee_joint_surplus = True
# --------------- Ende Seitenwahl / Preisanker ----------------
